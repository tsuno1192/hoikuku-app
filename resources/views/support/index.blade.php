        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-1 bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">児童一覧</h3>
                    <ul class="space-y-2">
                    ...
                    @foreach($children as $child)
                        <div id="child-{{ $child->id }}" class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-indigo-500">
                            ...
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h3 class="text-xl font-bold text-gray-900">{{ $child->name }} <span class="text-sm font-normal text-gray-500">({{ $child->birth_date }}生まれ)</span></h3>
                                    <p class="text-sm text-indigo-600 font-semibold mt-1">診断名: {{ $child->diagnosis ?? 'なし' }}</p>
                                </div>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg mb-6 text-sm">
                                <div>
                                    <span class="font-bold text-gray-700 block mb-1">⚡ 感覚過敏の傾向</span>
                                    <p class="text-gray-600 whitespace-pre-line">{{ $child->sensory_tendencies ?? '特になし' }}</p>
                                </div>
                                <div>
                                    <span class="font-bold text-gray-700 block mb-1">🚨 パニック時の対応手順</span>
                                    <p class="text-gray-600 whitespace-pre-line">{{ $child->panic_response_steps ?? '特になし' }}</p>
                                </div>
                            </div>
